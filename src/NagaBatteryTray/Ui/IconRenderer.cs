...
public static class IconRenderer
{
    ...
    public static Icon Render(DeviceState state, int dpi, bool gauge = true)
    {
        ...
        float ringW = 0f; // set below when the gauge style draws the rim; needed again to size its digits
        ...
        using (var g = Graphics.FromImage(bmp))
        {
            ...
            // Digits: the gauge style sits them INSIDE the coin, always white; the text style skips
            // the coin/ring and fills the whole icon at maximum size, colored by battery level. Lay
            // them out as a path so we can size by true ink bounds (digits have no descenders, so
            // MeasureString would leave dead vertical space).
            ...
            if (ink.Width > 0 && ink.Height > 0)
            {
                ...
                if (state.Status == DeviceStatus.Unknown)
                {
                    ...
                    // would stretch the thin hyphen into a slab filling the icon. Scale it
                ...
                {
                    // Text style: no coin/ring to clear — fill the icon height, condensing
                    // horizontally only when the digits are too wide (classic look, max size).
                    float margin = render * 0.04f;
                    float box = render - 2f * margin;
                    scaleY = box / ink.Height;
                    scaleX = ink.Width * scaleY > box ? box / ink.Width : scaleY;
                }
                ...
                // Gauge style fills + a same-color stroke: Segoe UI Bold is too light once the
                // digits are ~9 px tall on the final icon. Keep the stroke subtle — it also SHRINKS
                // the digit counters (the holes in 0/9), which is what kills legibility first; 3
                // digits get even less because their strokes sit closer together. Text style's
                // digits are already full-height, so no extra weight is needed — just fill.
                Color digitColor = gauge ? Color.White : ringColor;
                using var brush = new SolidBrush(digitColor);
                if (gauge)
                {
                    using var fat = new Pen(digitColor, render * (text.Length >= 3 ? 0.012f : 0.024f)) { LineJoin = LineJoin.Round };
                    g.DrawPath(fat, path);
                }
