...
- `Ui/` — `IconRenderer` (two switchable styles, `TrayIconStyle` setting, switchable live from the
  settings overlay: the default **coin gauge** — a filled dark disc (`Color.FromArgb(170, 16,
  ...
  sizes (16–24 px); or **"Text"** — the classic look, no coin/ring, digits fill the whole icon height
  and are colored by battery level instead of white); `TrayIcon` (raw
